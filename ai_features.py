...
def ai_chat():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('ai_chat.html', error="AI javob berishda xatolik yuz berdi")
    
    return render_template('ai_chat.html', chat_history=get_chat_history())
...
def update_ai_chat_route():
    """Update AI chat route to include topics"""
    print("=== UPDATING AI CHAT ROUTE ===")
    ...
    try:
        ...
        # Update the route to include topics
        if 'return render_template(\'ai_chat.html\', chat_history=get_chat_history())' in content:
            content = content.replace(
                'return render_template(\'ai_chat.html\', chat_history=get_chat_history())',
                'topics=Topic.query.all(), chat_history=get_chat_history())'
            )
            
            with open('app.py', 'w', encoding='utf-8') as f:
                f.write(content)
            
            print("AI chat route updated!")
            return True
        else:
            print("AI chat route not found or already updated")
            return True
            
    except Exception as e:
        print(f"Error updating AI chat route: {e}")
        return False
...
def main():
    ...
    # Update AI chat route
    if update_ai_chat_route():
        success_steps.append("AI Chat Route Update")
    else:
        print("Failed to update AI chat route")
        return False
